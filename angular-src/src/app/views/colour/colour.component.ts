...
export class ColourComponent implements OnInit {
  ...
  displayedColumns: string[] = ['ColourName','edit','delete'];
  ...
  colourRegister() {
    this.apiService.getData('/colours/colour', JSON.stringify(this.form.value)).then(d => {
      this.result = d;
      console.log(this.result);

      if ( this.result.success === true) {
        this.pop.snakbar('Colour Added', 'Successfully');

      } else {
      //  this.pop.snakbar('colour Register', 'Failed');
        this.pop.snakbar('Colour ID or Name is Already Exists', 'Failed');

      //  alert('colour Id or Name is Already Exists');
      }
  ...
  showColour() {
    this.apiService.retriveData('/colours/colour').then(displayColour => {
  ...
  deleteColour(value) {
    console.log(value);
    this.apiService.deleteData('/colours/colour/' + value).then(del => {
      this.delete = del;
      this.pop.snakbar('Colour Deleted', 'Successfully');
      this.showColour();
  ...
  openDialog(value): void {
    ...
    this.defRef.afterClosed().subscribe(result => {
      if (result != null) {
        ...
        this.apiService.updateData('/colours/colour/' + result.values._id, result.cData).then(d => {
          this.pop.snakbar('Colour Updated', 'Successfully');
          console.log(d);
          this.showColour();
        });
      }
