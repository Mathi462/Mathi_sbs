...
export class GstComponent implements OnInit {
  ...
  displayedColumns: string[] = ['CGST', 'SGST','edit','delete'];
  ...
  gstRegister() {
    this.apiService.getData('/gsts/gst', JSON.stringify(this.form.value)).then(d => {
      this.result = d;
      console.log(this.result);

      if ( this.result.success === true) {
        this.pop.snakbar('GST Added', 'Successfully');

      } else {
      //  this.pop.snakbar('GST Register', 'Failed');
        this.pop.snakbar('GST ID or Name is Already Exists', 'Failed');
  ...
  showGst() {
    this.apiService.retriveData('/gsts/gst').then(displayGst => {
      this.display = displayGst;
  ...
  deleteGst(value) {
    console.log(value);
    this.apiService.deleteData('/gsts/gst/' + value).then(del => {
      this.delete = del;
      this.pop.snakbar('GST Deleted', 'Successfully');
      this.showGst();
  ...
  openDialog(value): void {
    ...
    this.defRef.afterClosed().subscribe(result => {
      if (result != null) {
        ...
        this.apiService.updateData('/gsts/gst/' + result.values._id, result.cData).then(d => {
